    ...
    for root, dirs, files in os.walk(search_dir):
        for d in dirs:
            if d.find(bad_lccn) >= 0:
                if args.verbose:
                    print "  Found {}".format((root + '/' + d)[len(search_dir):])
...
def fix_lccns(effected_issue_path):
            ...
            if not args.dry_run:
                ...
                if not alto_file:
                    if not args.quiet and not f[-6:] == "_1.xml":
                        print "    Fix lccn in file {}".format(f)
                    ...
                    if not args.quiet and not f[-6:] == "_1.xml":
                        print "      LCCN XML Identifier: {}".format(lccn_xml.text)
    ...
    if not args.quiet:
        print "    Move {} to {}".format(bad_lccn, correct_lccn)
    ...
    if not os.path.exists(new_reel_path +'/'+ new_reel_path[-11:] +'.xml'):
        if not args.quiet:
            print "      Copy reel files from {} to {}".format(effected_reel_path_tail, new_reel_path_tail)
    ...
    for reel_path, reel_issues, _ in os.walk(effected_reel_path):
        if len(reel_issues) == 0:
            if not args.quiet:
                print "      Delete reel files no longer needed from {}".format(reel_path_tail_re.match(reel_path).group(1))
    ...
    # ---------------------------------------------
    if not args.quiet:
        print "    Update lccn in batch XML files to {}".format(correct_lccn)
    ...
    effected_issue_path_tail_re = re.compile(".+\/({}\/.+)$".format(bad_lccn))
        ...
        for issue in issues:
            if issue.text.find(effected_issue_path_tail) >= 0:
                ...
                if not args.quiet and not batch_file[-6:] == "_1.xml":
                    print "      Update issue {} replacing {} with {}".format(issue_date, bad_lccn, correct_lccn)
        ...
        if len(reels_copied) or len(reels_deleted):
            ...
            for reel_copied in reels_copied:
                ...
                for reel in reels:
                    ...
                    if (not copied_reel_added) and reel.text.find(reel_copied) >= 0:
                        if args.verbose and not batch_file[-6:] == "_1.xml":
                            print "      Copied reel {} already added to batch XML".format(reel_copied)
                ...
                if not copied_reel_added:
                    if not args.quiet and not batch_file[-6:] == "_1.xml":
                        print "      Adding copied reel {} to batch XML".format(reel_copied)
            ...
            for reel_deleted in reels_deleted:
                ...
                for reel in reels:
                    ...
                    if (not deleted_reel_removed) and reel.text.find(reel_deleted) >= 0:
                        if not args.quiet and not batch_file[-6:] == "_1.xml":
                            print "      Removing deleted reel {} found in batch XML".format(reel_deleted)
...
if  __name__ =='__main__':
    print "Searching for bad LCCN {} in\n{}".format(bad_lccn, search_dir)
    lccn_paths = find_lccn_paths()

    for d in lccn_paths:
        if not args.quiet:
            print "\nSearch for effected issues in:\n{}".format(d[len(search_dir):])
        effected_issue_paths = find_effected_issue_paths(d)

        for eip in effected_issue_paths:
            if not args.quiet:
                print "\n  Fix effected issue at {}".format(eip[(len(search_dir) + len(d[len(search_dir):])):])
            fix_lccns(eip)
